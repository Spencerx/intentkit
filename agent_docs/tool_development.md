...
- `intentkit/tools/mcp_myservice/schema.json` — tool states and config fields
...
1. **Discovery**: `McpCategoryModule.get_tools()` calls the MCP server to discover tools (cached for 1 hour).
2. **Filtering**: Only tools whose state is `"public"` or `"private"` (and matches `is_private`) are returned.
3. **Execution**: `McpToolTool._arun()` calls `call_mcp_tool()` which opens an MCP session, invokes the tool, and returns the text result.
