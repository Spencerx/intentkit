"""Card drawing tool using Deck of Cards API."""
...
try:
    ...
    raise ImportError(
        "httpx is required for Casino tools. Install it with: pip install httpx"
    )
from pydantic import BaseModel, Field

from intentkit.tools.casino.base import CasinoBaseTool
from intentkit.tools.casino.utils import (
    CURRENT_DECK_KEY,
    DECK_STORAGE_KEY,
    ENDPOINTS,
    RATE_LIMITS,
...
class CasinoDeckDraw(CasinoBaseTool):
    ...
    async def _arun(self, count: int = 1, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            deck_info = await self.get_agent_tool_data_raw(
