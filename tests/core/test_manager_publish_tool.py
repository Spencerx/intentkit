"""Tests for the manager-agent publish_agent tool."""
...
TOOL_MODULE = "intentkit.core.manager.tools.publish"


@pytest.fixture
def manager_runtime():
    """Mock the AgentContext seen by manager tools."""
...
def _stub_published_agent() -> AgentPublicInfo:
    """Return an AgentPublicInfo-shaped object the tool's serialiser can read."""
...
@pytest.mark.asyncio
async def test_publish_tool_forces_fee_one_and_passes_fields(manager_runtime):
    from intentkit.core.manager.tools.publish import publish_agent_tool

    with (
        patch(
            f"{TOOL_MODULE}.get_latest_public_info",
            new=AsyncMock(return_value=MagicMock()),
        ),
        patch(
            f"{TOOL_MODULE}.publish_agent",
            new=AsyncMock(return_value=_stub_published_agent()),
        ) as mock_publish,
    ):
        result = await publish_agent_tool._arun(  # pyright: ignore[reportPrivateUsage]
...
@pytest.mark.asyncio
async def test_publish_tool_omits_tags_when_none(manager_runtime):
    from intentkit.core.manager.tools.publish import publish_agent_tool
    ...
    with (
        ...
        await publish_agent_tool._arun(  # pyright: ignore[reportPrivateUsage]
...
@pytest.mark.asyncio
async def test_publish_tool_rejects_unknown_tag(manager_runtime):
    ...
    with (
        ...
        patch(f"{TOOL_MODULE}.publish_agent", new=AsyncMock()),
    ):
...
@pytest.mark.asyncio
async def test_publish_tool_handles_quota_reached(manager_runtime):
    from intentkit.core.manager.tools.publish import publish_agent_tool
...
@pytest.mark.asyncio
async def test_publish_tool_handles_agent_not_found(manager_runtime):
    from intentkit.core.manager.tools.publish import publish_agent_tool
...
@pytest.mark.asyncio
async def test_publish_tool_uses_agent_tag_enum_values():
